...
class Frame_model(nn.Module):
    def __init__(self, in_channels, out_channels, num_classes):
        super(Frame_model, self).__init__()
        ...
        # Bottleneck
        self.bottleneck_conv = conv3x3(48, 48)
        self.bottleneck_bn = nn.BatchNorm2d(48)
        self.bottleneck_relu = nn.LeakyReLU(0.1)

        # Classification Head
        self.classifier_fc = nn.Sequential(
            nn.Flatten(),
            nn.Linear(2352, 128),  # Adjust 8x8 if spatial size is different at bottleneck
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        # Bottleneck
        bottleneck = self.bottleneck_relu(self.bottleneck_bn(self.bottleneck_conv(out_pool5)))

        # Classification
        class_output = self.classifier_fc(bottleneck)
        ...
        # Final output
        recon_output = self.dec_act(out)

        return recon_output, class_output


class StudentFrameModel(nn.Module):
    """Student model based on EfficientNet-B0 with reduced capacity."""
    def __init__(self, in_channels, out_channels, num_classes):
        super(StudentFrameModel, self).__init__()
